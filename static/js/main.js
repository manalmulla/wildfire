...
const tileUrl = NASA_MAP_KEY && NASA_MAP_KEY !== "c0fd730a795a9fe1036b36f7526b4473"
  ? `https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png` // fallback (change if you have NASA tiles)
...
async function loadFiresAndRender() {
  try {
    ...
    points.forEach(p => {
      ...
      circle.on('click', () => {
        // when user clicks a point, show the popup (already bound) and also show toast
...
document.getElementById('centerMe').addEventListener('click', () => {
  ...
  navigator.geolocation.getCurrentPosition(pos => {
    ...
    L.circle([lat, lon], {radius: 1000, color: '#0077ff', fill: false}).addTo(map);
    checkNearbyAndNotify();
    // start periodic nearby check
    if (alertEnabled) {
...
async function checkNearbyAndNotify() {
  ...
  try {
    ...
    if (nearby.length > 0) {
      // show first few nearby in toast and native notification
      const nearest = nearby[0];
      const body = `${nearest.title}\nConfidence: ${nearest.confidence}\nDist: ${nearest.distance_km} km`;
      showToast(`<strong>Nearby wildfire!</strong><br/>${body}`, 8000);
      if (Notification.permission === "granted") {
        new Notification("Wildfire nearby", { body, tag: 'wildfire-alert' });
      }
    } else {
